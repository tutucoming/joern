...
/** Utility class that contains methods for creating CPG sub graphs.
  */
object AstSubGraphCreator {

  /** Create a global namespace block for the given `filename`
    */
  def globalNamespaceBlock(filename: String): NewNamespaceBlock = {
  ...
  def parameter(nameAndCode: String, index: Int): NewMethodParameterIn = NewMethodParameterIn()
    .code(nameAndCode)
    .order(index)
